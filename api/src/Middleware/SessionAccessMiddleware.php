...
class SessionAccessMiddleware implements MiddlewareInterface {
	 ...
	 * Process
	 *
	 * Checks access permission to create a stripe checkout session.
	 *
	...
	public function process( Request $request, Handler $handler ): Response {
		try {
			...
			$customerId = $request->getParsedBody()['stripe_customer_id'];

			$isVerifiedCustomer = $this
				->securityChecks
				->verifyCustomerAccess( $customerId, $userId );

			if ( $customerId && ! $isVerifiedCustomer ) {
			...
			$this->logger->error( 'Session access middleware: ' . $e->getMessage() );
