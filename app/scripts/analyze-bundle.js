...
function analyzeBundle() {
  ...
  if (readdirSync(DIST_PATH).includes('assets')) {
    ...
    const otherFiles = assets.filter(
      (file) => !jsFiles.includes(file) && !cssFiles.includes(file),
    )
  ...
  console.log('==========================')
  console.log(
    `Total Bundle Size: ${bundleAnalysis.analysis.summary?.totalSizeKB}KB`,
  )
  console.log(
    `JavaScript: ${bundleAnalysis.analysis.summary?.totalJSSizeKB}KB (${bundleAnalysis.analysis.summary?.totalJSFiles} files)`,
  )
  console.log(
    `CSS: ${bundleAnalysis.analysis.summary?.totalCSSSizeKB}KB (${bundleAnalysis.analysis.summary?.totalCSSFiles} files)`,
  )

  if (bundleAnalysis.analysis.performance?.largestFile) {
    console.log(
      `Largest File: ${bundleAnalysis.analysis.performance.largestFile.name} (${bundleAnalysis.analysis.performance.largestFile.sizeKB}KB)`,
    )
  }

  console.log(
    `Bundle Health: ${bundleAnalysis.analysis.performance?.bundleHealth}`,
  )
...
function getRecommendations(analysis) {
  ...
  if (analysis.summary?.totalJSFiles > 20) {
    recommendations.push(
      'Too many JavaScript files - consider better chunking strategy',
    )
  }

  if (
    analysis.javascript?.files &&
    analysis.javascript.files[0]?.sizeKB > 1000
  ) {
    recommendations.push(
      `Largest file is ${analysis.javascript.files[0].sizeKB}KB - consider splitting`,
    )
  }
