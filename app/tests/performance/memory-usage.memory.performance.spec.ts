...
test.describe('Memory Usage Monitoring', () => {
  ...
  test('baseline memory usage', async ({ page }) => {
    ...
    console.log(`✅ Baseline memory usage: ${Math.round(baselineMemory.usedJSHeapSize / (1024 * 1024))}MB`)
    console.log(`   Total heap: ${Math.round(baselineMemory.totalJSHeapSize / (1024 * 1024))}MB`)
    console.log(`   Heap limit: ${Math.round(baselineMemory.jsHeapSizeLimit / (1024 * 1024))}MB`)
  })
  ...
  test('memory usage during navigation', async ({ page }) => {
    ...
    for (const pageInfo of pages) {
      ...
      console.log(`   ${pageInfo.name}: ${usedMB}MB (${growthMB >= 0 ? '+' : ''}${growthMB}MB)`)
    }

    const report = createPerformanceReport('Navigation Memory Usage', {} as any, {
    ...
    console.log(`✅ Max memory growth during navigation: ${Math.round(maxGrowth / (1024 * 1024))}MB`)
  })

  test('memory leak detection through repeated navigation', async ({ page }) => {
    ...
    for (let cycle = 1; cycle <= navigationCycles; cycle++) {
      ...
      const currentMemory = await getMemoryInfo(page)
      const usedMB = Math.round(currentMemory.usedJSHeapSize / (1024 * 1024))
      const growthMB = Math.round((currentMemory.usedJSHeapSize - initialMemory.usedJSHeapSize) / (1024 * 1024))
      ...
      console.log(`     After cycle ${cycle}: ${usedMB}MB (${growthMB >= 0 ? '+' : ''}${growthMB}MB)`)
    }
    ...
    const report = createPerformanceReport('Memory Leak Detection', {} as any, {
      ...
      averageGrowthPerCycleMB: memoryReadings.length > 0 
        ? (memoryReadings[memoryReadings.length - 1]?.growthMB || 0) / navigationCycles 
        : 0
    })
    performanceReports.push(report)

    // Memory leak assertions
    const finalGrowthMB = memoryReadings[memoryReadings.length - 1]?.growthMB || 0
    ...
    console.log(`✅ Final memory growth: ${finalGrowthMB}MB (${averageGrowthPerCycle.toFixed(1)}MB per cycle)`)
  ...
  test('memory usage with large datasets', async ({ page }) => {
    ...
    for (const size of datasetSizes) {
      ...
      const currentMemory = await getMemoryInfo(page)
      const usedMB = Math.round(currentMemory.usedJSHeapSize / (1024 * 1024))
      const growthMB = Math.round((currentMemory.usedJSHeapSize - baselineMemory.usedJSHeapSize) / (1024 * 1024))
      ...
      console.log(`     ${size} lessons: ${usedMB}MB (${growthMB >= 0 ? '+' : ''}${growthMB}MB)`)
    ...
    const report = createPerformanceReport('Large Dataset Memory Usage', {} as any, {
      baselineMemoryMB: Math.round(baselineMemory.usedJSHeapSize / (1024 * 1024)),
      datasetMemoryUsage,
  ...
  test('component cleanup memory test', async ({ page }) => {
    ...
    cleanupTests.push({
      ...
      cleanupMB: Math.round((beforeNavAway.usedJSHeapSize - afterNavAway.usedJSHeapSize) / (1024 * 1024))
    })
    ...
    cleanupTests.push({
      component: 'Students',
      beforeMB: Math.round(beforeStudentsNavAway.usedJSHeapSize / (1024 * 1024)),
      afterMB: Math.round(afterStudentsNavAway.usedJSHeapSize / (1024 * 1024)),
      cleanupMB: Math.round((beforeStudentsNavAway.usedJSHeapSize - afterStudentsNavAway.usedJSHeapSize) / (1024 * 1024))
    })

    const report = createPerformanceReport('Component Cleanup Memory Test', {} as any, {
      initialMemoryMB: Math.round(initialMemory.usedJSHeapSize / (1024 * 1024)),
      cleanupTests,
      averageCleanupMB: cleanupTests.reduce((sum, test) => sum + test.cleanupMB, 0) / cleanupTests.length
    })
    performanceReports.push(report)

    // Component cleanup assertions
    cleanupTests.forEach(test => {
      console.log(`     ${test.component}: ${test.beforeMB}MB → ${test.afterMB}MB (${test.cleanupMB >= 0 ? '+' : ''}${test.cleanupMB}MB)`)
