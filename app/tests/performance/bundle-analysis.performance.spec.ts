...
test.describe('Bundle Size Impact Analysis', () => {
  ...
  test('initial bundle load analysis', async ({ page }) => {
    ...
    page.on('response', async (response) => {
      try {
        ...
        if (url.includes('.js') || url.includes('.css') || url.includes('chunk') || url.includes('vendor')) {
          const body = await response.body().catch(() => Buffer.alloc(0))
          
          networkRequests.push({
            url: url.replace(process.env.CI ? 'http://localhost:5000' : 'http://localhost:5173', ''),
    ...
    const jsFiles = networkRequests.filter(req => req.url.includes('.js') && !req.url.includes('node_modules'))
    ...
    const report = createPerformanceReport('Initial Bundle Load Analysis', metrics, {
      totalLoadTime,
      ...
      bundleAnalysis: {
        ...
        largestJSFile: jsFiles.length > 0 ? {
          url: jsFiles.sort((a, b) => b.size - a.size)[0].url,
          sizeKB: Math.round(jsFiles.sort((a, b) => b.size - a.size)[0].size / 1024)
        } : null
      },
      resourceBreakdown: {
  ...
  test('lazy loading performance', async ({ page }) => {
    ...
    const routes = [
      ...
      },
      {
        ...
        action: async () => {
          ...
          await waitForPageLoad(page)
          await expect(page.getByRole('heading', { name: 'Active students' })).toBeVisible()
          const endTime = Date.now()
    ...
    for (const route of routes) {
      ...
      console.log(`     ${route.name}: ${result.loadTime}ms, ${result.additionalSizeKB}KB additional`)
    }

    const metrics = await collectPerformanceMetrics(page)
    
    const report = createPerformanceReport('Lazy Loading Performance', metrics, {
      lazyLoadMetrics,
      averageLazyLoadTime: lazyLoadMetrics.reduce((sum, metric) => sum + metric.loadTime, 0) / lazyLoadMetrics.length,
      totalAdditionalSizeKB: lazyLoadMetrics.reduce((sum, metric) => sum + metric.additionalSizeKB, 0)
    })
    performanceReports.push(report)

    // Lazy loading assertions
    const averageLoadTime = lazyLoadMetrics.reduce((sum, metric) => sum + metric.loadTime, 0) / lazyLoadMetrics.length
  ...
  test('code splitting effectiveness', async ({ page }) => {
    ...
    const analyzeRoute = async (routeName: string, navigationAction: () => Promise<void>) => {
    ...
    const report = createPerformanceReport('Code Splitting Effectiveness', metrics, {
      totalUniqueResources: uniqueResources.length,
      sharedResources: {
        count: sharedResources.length,
        totalSizeKB: Math.round(sharedResources.reduce((sum, r) => sum + r.size, 0) / 1024)
      },
      routeSpecificResources: {
        count: routeSpecificResources.length,
        totalSizeKB: Math.round(routeSpecificResources.reduce((sum, r) => sum + r.size, 0) / 1024)
      },
      splittingEffectiveness: {
        sharedResourcesPercentage: Math.round((sharedResources.length / uniqueResources.length) * 100),
        routeSpecificPercentage: Math.round((routeSpecificResources.length / uniqueResources.length) * 100)
      },
      routeBreakdown: Object.entries(routeResourceMap).reduce((acc, [route, resources]) => {
        acc[route] = {
          totalResources: resources.length,
          totalSizeKB: Math.round(resources.reduce((sum, r) => sum + r.size, 0) / 1024),
    ...
    const splittingRatio = routeSpecificResources.length / uniqueResources.length
    ...
    console.log(`   Shared resources: ${sharedResources.length} (${Math.round((sharedResources.length / uniqueResources.length) * 100)}%)`)
    console.log(`   Route-specific resources: ${routeSpecificResources.length} (${Math.round((routeSpecificResources.length / uniqueResources.length) * 100)}%)`)
  })
  ...
  test('resource caching effectiveness', async ({ page }) => {
    ...
    const cacheHitRate = totalRequestCount > 0 ? (cacheHitCount / totalRequestCount) * 100 : 0
    const averageFirstLoad = cacheMetrics.reduce((sum, m) => sum + m.firstLoadTime, 0) / cacheMetrics.length
    const averageCachedLoad = cacheMetrics.reduce((sum, m) => sum + m.cachedLoadTime, 0) / cacheMetrics.length
    const cacheSpeedupRatio = averageCachedLoad > 0 ? averageFirstLoad / averageCachedLoad : 1
    
    const report = createPerformanceReport('Resource Caching Effectiveness', metrics, {
      ...
      resourceDetails: cacheMetrics.map(m => ({
        ...
        speedupRatio: Math.round((m.firstLoadTime / m.cachedLoadTime) * 100) / 100,
        cacheHit: m.cacheHit
      }))
    })
    performanceReports.push(report)
    ...
    console.log(`   Average speedup: ${Math.round(cacheSpeedupRatio * 100) / 100}x`)
