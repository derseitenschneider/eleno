...
test.describe('Large Dataset Performance', () => {
  ...
  test('pagination performance with large dataset', async ({ page }) => {
    ...
    for (let pageNum = 1; pageNum <= 3; pageNum++) {
      ...
      const nextButton = page.locator(
        '[aria-label="Go to next page"], button:has-text("Next")',
      )
    ...
    const report = createPerformanceReport('Pagination Performance', metrics, {
      datasetSize: 500,
      paginationTimes,
      averagePaginationTime:
        paginationTimes.length > 0
          ? paginationTimes.reduce((sum, p) => sum + p.time, 0) /
    ...
    if (paginationTimes.length > 0) {
      ...
      console.log(
        `✅ Average pagination time: ${Math.round(avgPaginationTime)}ms`,
      )
    }
  })
  ...
  test('search performance with large dataset', async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[placeholder*="Search"], input[placeholder*="Suche"]')
      .first()
    ...
    if (await searchInput.isVisible()) {
      ...
      for (const query of searchQueries) {
        ...
        console.log(
          `   Search "${query}": ${searchTime}ms (${resultCount} results)`,
        )
      ...
      const report = createPerformanceReport('Search Performance', metrics, {
        ...
        searchTimes,
        averageSearchTime:
          searchTimes.reduce((sum, s) => sum + s.time, 0) / searchTimes.length,
      })
      performanceReports.push(report)
      ...
      console.log(
        '⚠️ Search functionality not found, skipping search performance test',
      )
    }
  })
  ...
  test('sorting performance with large dataset', async ({ page }) => {
    ...
    if (headerCount > 0) {
      ...
      console.log(
        '⚠️ Sortable columns not found, skipping sorting performance test',
      )
    }
  })
})
