...
test.describe('Lesson Components Visual Regression Tests', () => {
  ...
  test('lesson header component - navigation and title', async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="lesson-header"], .lesson-header, h1, .page-title',
      {
        state: 'visible',
        timeout: 10000,
      },
    ...
    const headerLocator = page
      .locator('[data-testid="lesson-header"], .lesson-header')
      .first()
  ...
  test('lesson navigation sidebar - states and responsiveness', async ({
    page,
  }) => {
    const visual = createVisualTestHelper(page)
    ...
    await page.evaluate(() => {
      ...
      const isDarkTest = window.matchMedia(
  ...
  test('lesson item component - individual lesson display', async ({
    page,
  }) => {
    const visual = createVisualTestHelper(page)
  ...
  test('lesson mobile drawer - mobile-specific UI', async ({ page }) => {
    ...
    const drawerTrigger = page.locator(
      '[data-testid="mobile-drawer-trigger"], .mobile-menu-button, .hamburger-menu',
    )
  ...
  test('lesson content display - rich text and formatting', async ({
    page,
  }) => {
    const visual = createVisualTestHelper(page)
    ...
    const contentArea = page.locator(
      '[data-testid="lesson-content-display"], .lesson-content, .content-display',
    )
  ...
  test('homework display section - homework visualization', async ({
    page,
  }) => {
    const visual = createVisualTestHelper(page)

    // Look for homework display area
    const homeworkDisplay = page.locator(
      '[data-testid="homework-display"], .homework-display, .homework-section',
    )
  ...
  test('lesson notes component - note-taking interface', async ({ page }) => {
    ...
    const notesSection = page.locator(
      '[data-testid="lesson-notes"], .notes-section, .notes',
    )
          ...
          action: async () => {
            ...
            if (await noteInput.isVisible()) {
              await noteInput.fill(
                'Student showed improvement in rhythm. Work more on dynamics.',
              )
            }
          },
        },
  ...
  test('lesson action buttons - CRUD operations', async ({ page }) => {
    ...
    const actionButtons = page.locator(
      '[data-testid="lesson-actions"], .lesson-actions, .action-buttons',
    )
    ...
    if (await actionButtons.first().isVisible()) {
      ...
      for (let i = 0; i < Math.min(count, 3); i++) {
        ...
        await visual.testComponentStates(
          button,
          `lesson-action-${buttonText.toLowerCase().replace(/\s+/g, '-')}`,
          [
            {
              name: 'hover',
              action: async () => {
  ...
  test('lesson status indicators - visual feedback', async ({ page }) => {
    ...
    const statusIndicators = page.locator(
      '[data-testid="lesson-status"], .status-indicator, .lesson-status, .badge',
    )
  ...
  test('lesson search and filter - interface controls', async ({ page }) => {
    ...
    const searchFilter = page.locator(
      '[data-testid="lesson-search"], .search-input, input[type="search"], .filter-controls',
    )
