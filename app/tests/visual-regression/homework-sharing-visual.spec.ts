...
test.describe('Homework Sharing Visual Regression Tests', () => {
  ...
  test('homework sharing button - states and accessibility', async ({ page }) => {
    const visual = createVisualTestHelper(page)
    
    // Look for homework sharing button
    const shareButton = page.locator('[data-testid="share-homework"], button:has-text("Teilen"), button:has-text("Share"), .share-button')
      ...
      await visual.testComponentStates(button, 'homework-share-button', [
        ...
        },
        {
          ...
          action: async () => {
            ...
            const homeworkField = page.locator('[data-testid="homework"], textarea[placeholder*="Hausaufgabe"]')
  ...
  test('homework sharing modal - complete workflow', async ({ page }) => {
    ...
    if (await shareButton.first().isVisible()) {
      ...
      await page.waitForSelector('[role="dialog"], .modal, .share-modal', { state: 'visible' })
      
      const modal = page.locator('[role="dialog"], .modal, .share-modal').first()
      ...
      await visual.testComponentStates(modal, 'homework-sharing-modal', [
        ...
        },
        {
          ...
          action: async () => {
            ...
            await page.waitForSelector('.loading, .spinner', { state: 'detached', timeout: 5000 }).catch(() => {})
          }
        },
        {
          ...
          action: async () => {
            ...
            const expirationDropdown = modal.locator('select, [role="combobox"]')
  ...
  test('homework sharing link preview - generated link display', async ({ page }) => {
    const visual = createVisualTestHelper(page)
    ...
    if (await shareButton.first().isVisible()) {
      ...
      const generateButton = page.locator('button:has-text("Erstellen"), button:has-text("Generate"), button:has-text("Create"), .generate-button')
      if (await generateButton.isVisible()) {
        await generateButton.click()
        
        // Wait for link to be generated
        await page.waitForSelector('[data-testid="share-link"], .share-link, input[readonly]', { 
          state: 'visible',
          timeout: 10000 
        })
        
        const linkPreview = page.locator('[data-testid="share-link"], .share-link, input[readonly]').first()
  ...
  test('homework sharing options - configuration panel', async ({ page }) => {
    ...
    if (await shareButton.first().isVisible()) {
      ...
      const optionsPanel = page.locator('[data-testid="share-options"], .share-options, .sharing-settings')
      ...
      const expirationSelect = page.locator('select:has(option:has-text("Tag")), select:has(option:has-text("day")), [data-testid="expiration-select"]')
  ...
  test('homework sharing success state - confirmation UI', async ({ page }) => {
    ...
    if (await shareButton.first().isVisible()) {
      ...
      const generateButton = page.locator('button:has-text("Erstellen"), button:has-text("Generate"), button:has-text("Create")')
      if (await generateButton.isVisible()) {
        await generateButton.click()
        
        // Wait for success state
        await page.waitForSelector('[data-testid="share-success"], .success-message, .share-success', { 
          state: 'visible',
          timeout: 10000 
        }).catch(() => {})
        
        const successMessage = page.locator('[data-testid="share-success"], .success-message, .share-success')
  ...
  test('homework sharing error states - error handling UI', async ({ page }) => {
    const visual = createVisualTestHelper(page)
    
    // Mock network to simulate error
    await page.route('**/api/**', route => {
      if (route.request().method() === 'POST' && route.request().url().includes('share')) {
        route.fulfill({
          status: 500,
    ...
    if (await shareButton.first().isVisible()) {
      ...
      if (await generateButton.isVisible()) {
        ...
        await page.waitForSelector('[data-testid="share-error"], .error-message, .alert-error', { 
          state: 'visible',
          timeout: 10000 
        }).catch(() => {})
        
        const errorMessage = page.locator('[data-testid="share-error"], .error-message, .alert-error')
  ...
  test('homework expiration indicator - time-sensitive UI', async ({ page }) => {
    const visual = createVisualTestHelper(page)
    
    // Look for homework expiration indicators
    const expirationIndicator = page.locator('[data-testid="homework-expiration"], .expiration-indicator, .homework-expired')
    ...
    if (await expirationIndicator.first().isVisible()) {
      ...
      await visual.testComponentThemes(indicator, 'homework-expiration-indicator')
    }
  })

  test('homework sharing mobile flow - mobile-optimized UI', async ({ page }) => {
    const visual = createVisualTestHelper(page)
  ...
  test('homework sharing notifications - toast messages', async ({ page }) => {
    ...
    if (await shareButton.first().isVisible()) {
      ...
      const copyButton = page.locator('button:has-text("Kopieren"), button:has-text("Copy"), .copy-button')
      if (await copyButton.isVisible()) {
        await copyButton.click()
        
        // Wait for toast notification
        const toast = page.locator('[data-sonner-toaster], .toast, .notification')
