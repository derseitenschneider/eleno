...
test.describe('All Lessons Management Visual Regression Tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const allLessonsLink = page.locator(
      '[data-testid="all-lessons-link"], a:has-text("Alle Lektionen"), a:has-text("All Lessons")',
    )
  ...
  test('all lessons table - desktop layout', async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="lessons-table"], .lessons-table, table',
      {
        state: 'visible',
        timeout: 10000,
      },
    ...
    const lessonsTable = page
      .locator('[data-testid="lessons-table"], .lessons-table, table')
      .first()
    ...
    await visual.testComponentStates(lessonsTable, 'all-lessons-table', [
      {
        ...
        action: async () => {
          ...
          const sortableHeader = lessonsTable
            .locator('th[role="columnheader"], th button, .sortable-header')
            .first()
  ...
  test('all lessons mobile cards - mobile layout', async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="lesson-card"], .lesson-card, .mobile-card',
      {
        state: 'visible',
        timeout: 10000,
      },
  ...
  test('lessons filter controls - search and filter UI', async ({ page }) => {
    ...
    const filterControls = page.locator(
      '[data-testid="lessons-filter"], .filter-controls, .search-filter',
    )
      ...
      await visual.testComponentStates(controls, 'lessons-filter-controls', [
        {
          ...
          action: async () => {
            const searchInput = controls.locator(
        ...
        },
        {
          ...
          action: async () => {
            const dateFilter = controls.locator(
  ...
  test('lesson bulk actions - multi-select operations', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const bulkActions = page.locator(
        '[data-testid="bulk-actions"], .bulk-actions, .selected-actions',
      )
  ...
  test('lesson pagination - navigation controls', async ({ page }) => {
    ...
    if (await pagination.first().isVisible()) {
      ...
      await visual.testComponentStates(
        paginationControl,
        'lessons-pagination',
        [
          {
            name: 'page-2',
            action: async () => {
              const nextButton = paginationControl.locator(
                'button:has-text("2"), .page-2',
              )
  ...
  test('lesson export functionality - export UI', async ({ page }) => {
    ...
    const exportButton = page.locator(
      '[data-testid="export-lessons"], button:has-text("Export"), button:has-text("Exportieren")',
    )
    ...
    if (await exportButton.first().isVisible()) {
      ...
      const exportOptions = page.locator(
        '[data-testid="export-options"], .export-options, [role="dialog"]',
      )
  ...
  test('lesson table columns - customizable display', async ({ page }) => {
    ...
    const columnToggle = page.locator(
      '[data-testid="column-toggle"], .column-settings, button:has-text("Columns")',
    )
    ...
    if (await columnToggle.first().isVisible()) {
      ...
      const columnOptions = page.locator(
        '[data-testid="column-options"], .column-options, [role="menu"]',
      )
  ...
  test('lesson details drawer - expanded view', async ({ page }) => {
    ...
    const lessonRow = page
      .locator('[data-testid="lesson-row"], tr, .lesson-item')
      .first()
    ...
    if (await lessonRow.isVisible()) {
      ...
      const detailsDrawer = page.locator(
        '[data-testid="lesson-details"], .lesson-details, [role="dialog"]',
      )
  ...
  test('lesson status badges - visual indicators', async ({ page }) => {
    ...
    const statusBadges = page.locator(
      '[data-testid="lesson-status"], .status-badge, .badge',
    )
    ...
    if (await statusBadges.first().isVisible()) {
      ...
      for (let i = 0; i < Math.min(badgeCount, 5); i++) {
        ...
        await visual.takeScreenshot({
          name: `lesson-status-badge-${statusText
            .toLowerCase()
            .replace(/\s+/g, '-')}`,
          clip: badge,
        })
