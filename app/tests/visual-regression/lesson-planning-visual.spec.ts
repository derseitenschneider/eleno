...
test.describe('Lesson Planning Visual Regression Tests', () => {
  ...
  test('lesson planning modal - complete workflow', async ({ page }) => {
    ...
    const planningButton = page.locator('[data-testid="planning-modal-button"], button:has-text("Planen")')
      ...
      await visual.testComponentStates(modalLocator, 'lesson-planning-modal', [
        {
          ...
          action: async () => {
            ...
            const contentArea = page.locator('[data-testid="lesson-content"], textarea, [contenteditable="true"]').first()
  ...
  test('planned lesson items - list view', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="planned-lessons-list"], .planned-lessons', { 
      state: 'visible', 
      timeout: 10000 
    })
    
    const listLocator = page.locator('[data-testid="planned-lessons-list"], .planned-lessons').first()
  ...
  test('lesson planning dropdown - states', async ({ page }) => {
    ...
    const dropdownTrigger = page.locator('[data-testid="lesson-dropdown"], [role="button"]:has-text("⋮"), .dropdown-trigger')
  ...
  test('lesson content editor - visual states', async ({ page }) => {
    ...
    const editorLocator = page.locator('[data-testid="lesson-content"], .editor, [contenteditable="true"]')
    ...
    if (await editorLocator.first().isVisible()) {
      ...
      await visual.testComponentStates(editor, 'lesson-content-editor', [
        ...
        },
        {
          name: 'with-content',
          action: async () => {
            await editor.fill('This is sample lesson content for testing the visual appearance of the editor component.')
          }
        }
      ])
    }
  ...
  test('lesson date picker - calendar component', async ({ page }) => {
    ...
    const datePickerTrigger = page.locator('input[type="date"], [data-testid="date-picker"], button:has-text("Datum")')
    ...
    if (await datePickerTrigger.first().isVisible()) {
      ...
      if (await datePicker.getAttribute('type') !== 'date') {
        ...
        const calendar = page.locator('[role="dialog"] .calendar, .date-picker-calendar')
  ...
  test('lesson planning toolbar - action buttons', async ({ page }) => {
    ...
    const toolbar = page.locator('[data-testid="lesson-toolbar"], .lesson-actions, .toolbar')
    ...
    if (await toolbar.first().isVisible()) {
      ...
      // Test responsive design
      await visual.testResponsiveComponent(toolbarElement, 'lesson-planning-toolbar')
    }
  })
  ...
  test('homework assignment section - visual layout', async ({ page }) => {
    ...
    const homeworkSection = page.locator('[data-testid="homework"], .homework-section, textarea[placeholder*="Homework"], textarea[placeholder*="Hausaufgabe"]')
    ...
    if (await homeworkSection.first().isVisible()) {
      ...
      await visual.testComponentStates(homework, 'homework-section', [
        ...
        },
        {
          name: 'filled',
          action: async () => {
            await homework.fill('Practice scales: C major, G major, and D major. Focus on proper fingering and tempo control.')
          }
        }
      ])
    }
