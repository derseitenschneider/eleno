...
test.describe('Keyboard Navigation Accessibility Tests', () => {
  ...
  test.describe('Main Navigation Keyboard Accessibility', () => {
    test('should navigate through main sidebar using keyboard', async ({ page }) => {
      await page.goto('/')
    ...
    test('should handle Enter key activation on navigation items', async ({ page }) => {
      await page.goto('/')
    ...
    test('should handle Space key activation on navigation buttons', async ({ page }) => {
      await page.goto('/')
      ...
      await expect(page.getByRole('heading', { name: 'studenten' })).toBeVisible()
    })
  })

  test.describe('Lesson Management Keyboard Navigation', () => {
    test('should navigate lesson creation form with keyboard', async ({ page }) => {
      ...
      expect(formTabStops.some(stop => stop.includes('lesson-title'))).toBe(true)
      expect(formTabStops.some(stop => stop.includes('lesson-notes'))).toBe(true)
    })

    test('should handle Escape key to close lesson modals', async ({ page }) => {
      await lessonsPOM.goto()
    ...
    test('should navigate lesson items list with arrow keys', async ({ page }) => {
      await lessonsPOM.goto()
      ...
      const firstLessonItem = page.locator('[data-testid*="lesson-item-"]').first()
      ...
      const secondLessonItem = page.locator('[data-testid*="lesson-item-"]').nth(1)
  ...
  test.describe('Modal Dialog Focus Management', () => {
    ...
    test('should trap focus within lesson update modal', async ({ page }) => {
      ...
      const lessonModal = page.locator('[data-testid*="lesson-modal"], [data-testid*="lesson-details"], .lesson-modal')
      if (await lessonModal.count() > 0) {
        await accessibilityHelpers.testFocusTrapping(await lessonModal.first().getAttribute('data-testid') || '.lesson-modal')
      }
    })
  ...
  test.describe('Student Management Keyboard Navigation', () => {
    test('should navigate student table with keyboard', async ({ page }) => {
      ...
      expect(tabStops.some(stop => stop.includes('student') || stop.includes('table'))).toBe(true)
    })

    test('should handle Enter key on student action buttons', async ({ page }) => {
      await studentsPOM.goto()
  ...
  test.describe('Form Field Keyboard Navigation', () => {
    test('should navigate form fields with Tab and Shift+Tab', async ({ page }) => {
      await lessonsPOM.goto()
    ...
    test('should handle form submission with Enter key', async ({ page }) => {
      ...
      const titleField = page.locator('[data-testid*="title"], input[name="title"], input[placeholder*="titel"]').first()
    ...
    test('should support standard keyboard shortcuts', async ({ page }) => {
      ...
      for (const shortcut of shortcuts) {
        try {
          ...
          console.log(`Keyboard shortcut ${shortcut.keys}: ${actionExecuted ? 'WORKS' : 'NOT_IMPLEMENTED'}`)
        } catch (error) {
  ...
  test.describe('Skip Links and Quick Navigation', () => {
    test('should provide skip links for keyboard users', async ({ page }) => {
      ...
      // Look for skip links
      const skipLinks = page.locator('a[href*="#main"], a[href*="#content"], [data-testid*="skip"]')
      ...
      if (skipLinkCount > 0) {
        ...
        console.log('Skip links not found - consider adding for better accessibility')
      }
    })
  })
