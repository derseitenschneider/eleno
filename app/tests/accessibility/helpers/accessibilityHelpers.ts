...
export class AccessibilityHelpers {
  ...
  async testKeyboardNavigation(options?: {
    ...
    while (iterations < maxIterations) {
      ...
      const elementId =
        (await newFocusedElement.getAttribute('data-testid')) ||
  ...
  async testFocusTrapping(dialogSelector: string) {
    ...
    if (count === 0) {
      throw new Error(
        `No focusable elements found in dialog: ${dialogSelector}`,
      )
    }
  ...
  async validateAriaLabels(selector: string) {
    ...
    for (const element of elements) {
      ...
      if (
        ...
        ['button', 'link', 'textbox', 'combobox', 'listbox'].includes(
          role || '',
        )
      ) {
        const hasLabel =
        ...
        if (!hasLabel) {
          ...
          throw new Error(
            `Interactive element missing accessible name: ${JSON.stringify(
              elementInfo,
            )}`,
          )
        }
    ...
    if (liveRegionCount > 0) {
      for (let i = 0; i < liveRegionCount; i++) {
        ...
        announcements.push({
          ...
          content,
          atomic: atomic === 'true',
          selector: await region.evaluate(
            (el) =>
              el.tagName +
  ...
  async testKeyboardShortcuts(
    ...
    for (const shortcut of shortcuts) {
      ...
      if (!actionExecuted) {
        throw new Error(
          `Keyboard shortcut failed: ${shortcut.keys} for ${shortcut.expectedAction}`,
        )
      }
    }
