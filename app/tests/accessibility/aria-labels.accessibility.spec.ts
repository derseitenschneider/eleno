...
test.describe('ARIA Label Validation Tests', () => {
  ...
  test.describe('Interactive Elements ARIA Labels', () => {
    test('should validate button ARIA labels and accessible names', async ({
      page,
    }) => {
      await page.goto('/')
      await expect(
      ...
      for (let i = 0; i < Math.min(buttonCount, 20); i++) {
        ...
        if (isVisible) {
          ...
          const isIconButton =
            !hasTextContent && (await button.locator('svg, .icon').count()) > 0

          if (isIconButton && !ariaLabel && !ariaLabelledBy) {
            console.log(`Icon button ${i} missing aria-label:`, {
              testId,
    ...
    test('should validate input field labels and associations', async ({
      ...
      for (let i = 0; i < fieldCount; i++) {
        ...
        const hasAccessibleName =
          ariaLabel || hasAssociatedLabel || hasReferencedLabel || placeholder
        ...
        if (!hasAccessibleName) {
          console.log(`Form field ${i} missing accessible name:`, {
            fieldType,
            fieldName,
            fieldId,
          })
      ...
      const fieldsWithAccessibleNames = fieldResults.filter(
        (f) => f.hasAccessibleName,
      ).length
      const totalFields = fieldResults.length

      if (totalFields > 0) {
        console.log(
          `Form field accessibility: ${fieldsWithAccessibleNames}/${totalFields} fields have accessible names`,
        )
    ...
    test('should validate link accessibility and context', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(linkCount, 15); i++) {
        ...
        if (isVisible) {
          ...
          if (!hasAccessibleName) {
            console.log(`Link ${i} missing accessible name:`, {
              href,
              linkText,
            })
          }
          ...
          if (isExternalLink && !hasExternalIndicator) {
            console.log(`External link ${i} missing indicator:`, {
              linkText,
              href,
            })
          }
      ...
      const linksWithAccessibleNames = linkResults.filter(
      ...
      if (totalVisibleLinks > 0) {
        console.log(
          `Link accessibility: ${linksWithAccessibleNames}/${totalVisibleLinks} links have accessible names`,
        )
  ...
  test.describe('Dynamic Content ARIA States', () => {
    test('should validate modal dialog ARIA attributes', async ({ page }) => {
      ...
      const modal = page
        .getByTestId('create-lesson-form')
        .locator(
          'xpath=ancestor-or-self::*[contains(@class, "dialog") or contains(@class, "modal") or @role="dialog" or @role="alertdialog"]',
        )
        .first()
      ...
      if ((await modal.count()) > 0) {
        ...
        if (!hasAccessibleName) {
          console.log(
            'Modal missing accessible name (aria-label or aria-labelledby)',
          )
        }
    ...
    test('should validate dropdown/combobox ARIA states', async ({ page }) => {
      ...
      const comboboxes = page.locator(
        '[role="combobox"], select, [data-testid*="select"], [data-testid*="dropdown"]',
      )
    ...
    test('should validate expandable content ARIA states', async ({ page }) => {
      ...
      const expandableElements = page.locator(
        '[aria-expanded], button[data-testid*="expand"], button[data-testid*="toggle"], [data-testid*="accordion"]',
      )
      ...
      for (let i = 0; i < Math.min(expandableCount, 10); i++) {
        ...
        if (isVisible) {
          ...
          if ((await element.count()) > 0 && ariaExpanded !== null) {
            ...
            console.log(`Expandable element ${i} state change:`, {
              initialState,
              newState,
              stateChanged,
            })
            ...
            if (!stateChanged) {
              console.log(
                `Expandable element ${i} aria-expanded state not updating`,
              )
            }
          }
        }
  ...
  test.describe('Form Validation ARIA Messages', () => {
    test('should validate error message ARIA associations', async ({
      page,
    }) => {
      await lessonsPOM.goto()
      ...
      if ((await submitButton.count()) > 0) {
        ...
        const errorMessages = page.locator(
          '[role="alert"], [aria-live="assertive"], .error-message, [data-testid*="error"], .text-red-500, .error',
        )
        ...
        if (errorCount > 0) {
          for (let i = 0; i < errorCount; i++) {
            ...
            if (isVisible) {
              ...
              if (errorId) {
                const referencingField = page.locator(
                ...
                if (!hasReferencingField) {
                  console.log(
                    `Error message ${i} (${errorId}) not referenced by any field`,
                  )
                }
              }
              ...
              if (
                ...
                console.log(
                  `Error message ${i} missing announcement attributes`,
                )
              }
            }
          }
          ...
          console.log(
            'No validation error messages found (form may be valid or validation not implemented)',
          )
        }
      }
    })

    test('should validate success message ARIA announcements', async ({
      page,
    }) => {
      await lessonsPOM.goto()
      ...
      const successMessages = page.locator(
        '[role="status"], [aria-live], .success-message, [data-testid*="success"], .text-green-500, .success, .toast',
      )
      ...
      if (successCount > 0) {
        for (let i = 0; i < successCount; i++) {
          ...
          if (isVisible) {
            ...
            if (!hasAnnouncement) {
              console.log(
                `Success message ${i} missing announcement attributes`,
              )
            }
          }
        }
  ...
  test.describe('Navigation and Breadcrumb ARIA', () => {
    test('should validate navigation ARIA structure', async ({ page }) => {
      ...
      for (let i = 0; i < navCount; i++) {
        ...
        if (isVisible) {
          ...
          const currentItems = nav.locator(
            '[aria-current="page"], [aria-current="true"], .current, .active',
          )
          ...
          if (currentCount > 0) {
            for (let j = 0; j < currentCount; j++) {
              ...
              console.log(`Current page indicator ${j}:`, {
                ariaCurrent,
    ...
    test('should validate breadcrumb ARIA structure', async ({ page }) => {
      ...
      const breadcrumbs = page.locator(
        'nav[aria-label*="breadcrumb"], nav[aria-label*="Breadcrumb"], [role="navigation"][aria-label*="breadcrumb"], ol.breadcrumb, ul.breadcrumb, [data-testid*="breadcrumb"]',
      )
      ...
      if (breadcrumbCount > 0) {
        for (let i = 0; i < breadcrumbCount; i++) {
          ...
          if (isVisible) {
            ...
            if (itemCount > 0) {
              ...
              console.log(
                `Breadcrumb ${i} has ${itemCount} items, last item aria-current: ${ariaCurrent}`,
              )

              if (!ariaCurrent) {
                console.log(
                  `Breadcrumb ${i} last item should have aria-current="page"`,
                )
              }
            }
          }
        ...
        console.log(
          'No breadcrumbs found - consider adding for complex navigation',
        )
      }
    })
  })
