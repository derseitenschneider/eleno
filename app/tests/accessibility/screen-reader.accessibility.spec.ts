...
test.describe('Screen Reader Compatibility Tests', () => {
  ...
  test.describe('Semantic HTML Structure', () => {
    test('should have proper heading hierarchy', async ({ page }) => {
      ...
      const semanticStructure = await accessibilityHelpers.validateSemanticStructure()
    ...
    test('should have proper landmark regions on lessons page', async ({ page }) => {
    ...
    test('should have proper landmark regions on students page', async ({ page }) => {
  ...
  test.describe('ARIA Live Regions and Announcements', () => {
    test('should announce dynamic content changes', async ({ page }) => {
      ...
      const initialAnnouncements = await accessibilityHelpers.testScreenReaderAnnouncements()
      ...
      const postActionAnnouncements = await accessibilityHelpers.testScreenReaderAnnouncements()
      
      // Should have aria-live regions for announcements
      const hasLiveRegions = postActionAnnouncements.length > 0 || 
                            await page.locator('[aria-live], [role="status"], [role="alert"]').count() > 0
      
      // Note: Not failing test if no live regions, but logging for improvement
      if (!hasLiveRegions) {
        console.log('Consider adding aria-live regions for better screen reader experience')
      }
    })
    ...
    test('should announce form validation errors', async ({ page }) => {
      ...
      if (await submitButton.count() > 0) {
        ...
        const errorMessages = page.locator('[role="alert"], [aria-live="assertive"], .error-message, [data-testid*="error"]')
        ...
        if (errorCount > 0) {
          ...
          for (let i = 0; i < Math.min(errorCount, 3); i++) {
            ...
            if (!hasAccessibilityAttribute) {
              console.log('Error message missing accessibility attributes:', await error.textContent())
            }
          }
        }
      }
    ...
    test('should announce loading states', async ({ page }) => {
      ...
      const loadingIndicators = page.locator('[aria-busy="true"], [role="status"], .loading, [data-testid*="loading"]')
      ...
      if (loadingCount > 0) {
        ...
        for (let i = 0; i < Math.min(loadingCount, 3); i++) {
          ...
          const hasAriaDescribedBy = await loader.getAttribute('aria-describedby')
          
          // Loading states should be announced to screen readers
          const hasAccessibilitySupport = hasAriaLabel || hasRole === 'status' || hasLive || hasAriaDescribedBy
  ...
  test.describe('Form Field Associations', () => {
    test('should properly associate form labels with inputs', async ({ page }) => {
      await lessonsPOM.goto()
    ...
    test('should provide helpful field descriptions', async ({ page }) => {
      ...
      const helpTexts = page.locator('[data-testid*="help"], [data-testid*="description"], .help-text, .field-description')
      ...
      if (helpCount > 0) {
        for (let i = 0; i < helpCount; i++) {
          ...
          if (isVisible && helpId) {
            ...
            const referencingInput = page.locator(`[aria-describedby*="${helpId}"]`)
            const hasReferencingInput = await referencingInput.count() > 0
            
            if (!hasReferencingInput) {
              console.log('Help text not referenced by any input:', await helpText.textContent())
            }
          }
        }
      }
  ...
  test.describe('Interactive Element Descriptions', () => {
    test('should provide clear button descriptions', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(buttonCount, 10); i++) {
        ...
        const hasDescription = (buttonText && buttonText.trim().length > 0) || 
    ...
    test('should provide clear link descriptions', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(linkCount, 10); i++) {
        ...
        const hasDescription = (linkText && linkText.trim().length > 0) || 
                              ariaLabel || ariaLabelledBy || title
        
        if (!hasDescription) {
          const href = await link.getAttribute('href')
          console.log('Link missing description:', { href, className: await link.getAttribute('class') })
        }
      }
    })
  })
  ...
  test.describe('Content Reading Order', () => {
    test('should have logical reading order on main pages', async ({ page }) => {
      await page.goto('/')
      await expect(page.getByRole('heading', { name: 'Quick Links' })).toBeVisible()
      
      // Simulate screen reader navigation
      const navigationResults = await accessibilityHelpers.simulateScreenReaderNavigation()
      
      // Should be able to navigate through content
      const successfulNavigations = navigationResults.filter(result => result.navigationSuccessful)
          ...
          if (count > 1) {
            for (let i = 0; i < count; i++) {
              ...
              if (!ariaLabel && !ariaLabelledBy) {
                console.log(`Multiple ${landmarkType} landmarks should have distinguishing labels`)
              }
            }
          }
        }
