...
test.describe('Comprehensive Accessibility Tests', () => {
  ...
  test.describe('Lesson Planning Workflow Accessibility', () => {
    test('should provide accessible lesson planning interface', async ({
      ...
      await accessibilityHelpers.scanComponent(
    ...
    test('should provide accessible lesson planning components', async ({
      page,
    }) => {
      await lessonsPOM.goto()
  ...
  test.describe('Homework Sharing Interface Accessibility', () => {
    test('should provide accessible homework sharing modals', async ({
      page,
    }) => {
      await lessonsPOM.goto()
      ...
      const homeworkButtons = page.locator(
        'button:has-text("homework"), button[data-testid*="homework"], button:has-text("share"), [data-testid*="share"]',
      )
      ...
      if (homeworkButtonCount > 0) {
        ...
        const homeworkModal = page.locator(
          '[data-testid*="homework"], [data-testid*="share"], .homework-modal, .share-modal',
        )
        ...
        if ((await homeworkModal.count()) > 0) {
          ...
          await accessibilityHelpers.scanComponent(
            (await homeworkModal.first().getAttribute('data-testid')) ||
              '.homework-modal',
          )
          ...
          if (await homeworkModal.first().getAttribute('data-testid')) {
            await accessibilityHelpers.testFocusTrapping(
              `[data-testid="${await homeworkModal
                .first()
          ...
          const formFields = homeworkModal.locator(
          ...
          if (fieldCount > 0) {
            ...
            await accessibilityHelpers.validateAriaLabels(
              'input, textarea, select',
            )
          }
        } else {
          console.log(
            'Homework sharing interface not found after clicking button',
          )
        }
      } else {
        console.log(
          'No homework sharing buttons found - this may be expected based on current lesson state',
        )
      }
  ...
  test.describe('All Lessons Management Accessibility', () => {
    test('should provide accessible lessons management table', async ({
      page,
    }) => {
      ...
      if (tableCount > 0) {
        ...
        for (let i = 0; i < tableCount; i++) {
          ...
          console.log(`Table ${i} accessibility:`, {
            hasCaption,
            ariaLabel,
            ariaLabelledBy,
          })
      ...
      const controls = page.locator(
        'button, [role="button"], input[type="search"], select',
      )
    ...
    test('should provide accessible lessons table sorting and filtering', async ({
      page,
    }) => {
      await page.goto('/lessons/all')
      ...
      const sortableHeaders = page.locator(
        'th[role="columnheader"], th button, th[aria-sort]',
      )
      ...
      if (sortableCount > 0) {
        ...
        for (let i = 0; i < Math.min(sortableCount, 3); i++) {
          ...
          console.log(
            `Sortable header ${i}: "${headerText}", aria-sort: ${ariaSort}, role: ${role}`,
          )
      ...
      if (searchCount > 0) {
        ...
        for (let i = 0; i < searchCount; i++) {
          ...
          const associatedLabel = await page
            .locator(`label[for="${await searchInput.getAttribute('id')}"]`)
            .count()
  ...
  test.describe('Navigation and Sidebar Accessibility', () => {
    test('should provide accessible main navigation', async ({ page }) => {
        ...
        if (isVisible) {
          ...
          for (let j = 0; j < Math.min(linkCount, 5); j++) {
            ...
            if (await navItem.isVisible()) {
              ...
              console.log(
                `Nav item ${j}: "${itemText}", aria-current: ${ariaCurrent}, href: ${href}, testId: ${testId}`,
              )
      ...
      const sidebarElements = page.locator(
        '[data-testid*="sidebar"], [data-testid*="nav"], .sidebar',
      )
    ...
    test('should provide accessible sidebar interactions', async ({ page }) => {
      ...
      const sidebarNavItems = page.locator(
        '[data-testid*="nav-sidebar"], [data-testid*="sidebar"]',
      )
  ...
  test.describe('Form and Input Accessibility', () => {
    test('should provide accessible form interactions across the application', async ({
      page,
    }) => {
      ...
      const studentButtons = page.locator(
        'button:has-text("New"), button:has-text("Neu"), button[data-testid*="create"]',
      )
  ...
  test.describe('Mobile and Responsive Accessibility', () => {
    test('should maintain accessibility on mobile viewports', async ({
      ...
      const mobileNavButtons = page.locator(
        '[data-testid*="mobile"], button[aria-expanded], .hamburger, .menu-toggle',
      )
