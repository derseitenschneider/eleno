...
test.describe('Color Contrast Verification Tests', () => {
  ...
  test.describe('Text Color Contrast', () => {
    test('should verify text contrast ratios on main dashboard', async ({ page }) => {
      await page.goto('/')
      ...
      const textElements = page.locator('h1, h2, h3, h4, p, span:visible, button, a, label')
      ...
      const lightColorElements = contrastResults.filter(r => 
      ...
      if (lightColorElements.length > 0) {
        console.log(`Found ${lightColorElements.length} elements with light text colors`)
      }
    })
    ...
    test('should verify contrast in dark mode', async ({ page }) => {
      ...
      const darkModeToggle = page.locator('[data-testid*="dark"], [data-testid*="theme"], button:has-text("Dark"), button:has-text("Theme")').first()
      ...
      if (await darkModeToggle.count() > 0) {
        ...
        const isDarkMode = (bodyClass?.includes('dark') || htmlClass?.includes('dark')) || 
        ...
        console.log('Dark mode toggle not found - skipping dark mode contrast test')
      }
    })

    test('should verify button and interactive element contrast', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(buttonCount, 15); i++) {
        ...
        try {
          ...
          if (buttonInfo.backgroundColor === 'rgba(0, 0, 0, 0)' || 
      ...
      console.log(`Analyzed ${buttonContrastResults.length} buttons for contrast`)
      
      // Test specific button states
      const primaryButtons = buttonContrastResults.filter(b => 
        b.className?.includes('primary') || 
        b.className?.includes('bg-blue') ||
        b.className?.includes('bg-green')
      )
      
      const secondaryButtons = buttonContrastResults.filter(b => 
        b.className?.includes('secondary') || 
        b.className?.includes('outline') ||
        b.className?.includes('ghost')
      )
      
      console.log(`Found ${primaryButtons.length} primary buttons, ${secondaryButtons.length} secondary buttons`)
    })

    test('should verify form field contrast and focus indicators', async ({ page }) => {
      await lessonsPOM.goto()
    ...
    test('should verify navigation element contrast', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(navCount, 8); i++) {
        ...
        if (isVisible) {
          ...
          const isActive = await navItem.getAttribute('aria-current') === 'page' ||
    ...
    test('should verify icon visibility and contrast', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(iconCount, 15); i++) {
        ...
        if (isVisible) {
          ...
          if (!hasAdequateSize) {
            console.log(`Icon ${i} may be too small: ${iconInfo.width}x${iconInfo.height}`)
          }
    ...
    test('should verify image alt text and accessibility', async ({ page }) => {
      ...
      for (let i = 0; i < imageCount; i++) {
        ...
        if (isVisible) {
          ...
          console.log(`Image ${i}:`, { src, alt, ariaLabel, ariaLabelledBy, role })
  ...
  test.describe('Error and Status Message Contrast', () => {
    test('should verify error message contrast', async ({ page }) => {
      ...
      if (await submitButton.count() > 0) {
        ...
        const errorMessages = page.locator('.error, .text-red-500, [data-testid*="error"], [role="alert"]')
        ...
        for (let i = 0; i < errorCount; i++) {
          ...
          if (isVisible) {
            ...
            const isRedText = errorStyles.color.includes('rgb(239') || // red-500 range
    ...
    test('should verify success message contrast', async ({ page }) => {
      ...
      const statusMessages = page.locator('.success, .text-green-500, [data-testid*="success"], [role="status"], .toast')
  ...
  test.describe('High Contrast Mode Testing', () => {
    test('should verify high contrast mode compatibility', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(buttonCount, 10); i++) {
        ...
        const hasBackground = buttonStyles.backgroundColor !== 'rgba(0, 0, 0, 0)' && 
