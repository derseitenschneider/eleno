...
function generatePoolKey(type: string, overrides?: any): string {
  ...
  const keyParts = sortedKeys.map(key => `${key}=${JSON.stringify(overrides[key])}`)
...
function getFromPool<T>(
  ...
  factoryStats.averageCreationTime = factoryStats.totalCreationTime / factoryStats.poolMisses
 ...
 */
export function createMinimalStudent(id = 1): Pick<DbStudent, 'id' | 'firstName' | 'lastName'> {
  return {
    id,
    firstName: 'Test',
    lastName: 'Student',
...
export function createMinimalLesson(id = 1): Pick<any, 'id' | 'date' | 'status'> {
  return {
    id,
    date: new Date('2023-12-01'),
    status: 'documented',
...
export function printFactoryReport() {
  ...
  console.log(`  • Average creation time: ${factoryStats.averageCreationTime.toFixed(3)}ms`)
  console.log(`  • Memory reused: ${(factoryStats.memoryReused / 1024).toFixed(2)}KB`)
  
  console.log('\n  Pool sizes:')
  for (const [type, pool] of Object.entries(objectPools)) {
    const totalObjects = Array.from(pool.values()).reduce((sum, arr) => sum + arr.length, 0)
    console.log(`    • ${type}: ${totalObjects} objects in ${pool.size} variants`)
  }
}
...
export function createMockStudents(count = 3): DbStudent[] {
  return Array.from({ length: count }, (_, index) => createMockStudent({ id: index + 1 }))
}

export function createMockGroups(count = 3): DbGroup[] {
  return Array.from({ length: count }, (_, index) => createMockGroup({ id: index + 1 }))
}

export function createMockLessons(count = 3): any[] {
  return Array.from({ length: count }, (_, index) => createMockLesson({ id: index + 1 }))
}

export function createMockNotes(count = 3): DbNote[] {
  return Array.from({ length: count }, (_, index) => createMockNote({ id: index + 1 }))
}

export function createMockTodos(count = 3): DbTodo[] {
  return Array.from({ length: count }, (_, index) => createMockTodo({ id: index + 1 }))
}
