import type { TStudent, TWeekday } from "../types/types";
import { sortStudentsDateTime } from "./sortStudents";

const calcNearestStudentIndex = (students: TStudent[]) => {
  ...
  const nearestStudentIndex = filteredSortedStudents.findIndex(
    (student) => student.id === upcomingStudent?.id,
  );

  if (nearestStudentIndex === -1) return 0;
