...
describe('AttendanceStatusSelect', () => {
  it('renders the select trigger (kebab icon) and shows proper badge when value changes', () => {
    ...
    const { rerender } = render(
      <AttendanceStatusSelect value='held' onChange={handleChange} />,
    )
    ...
    expect(
      screen.queryByText('Schülerabsenz (entschuldigt)'),
    ).not.toBeInTheDocument()
    expect(
      screen.queryByText('Schülerabsenz (unentschuldigt)'),
    ).not.toBeInTheDocument()
    ...
    rerender(
      <AttendanceStatusSelect value='teacher_absent' onChange={handleChange} />,
    )
    expect(screen.getByText('Lehrerabsenz')).toBeInTheDocument()
  })

  it('is disabled when the disabled prop is true', () => {
    render(<AttendanceStatusSelect value='held' onChange={() => {}} disabled />)
  ...
  it('displays the correct badges for different absence types', () => {
    ...
    expect(
      screen.getByText('Schülerabsenz (unentschuldigt)'),
    ).toBeInTheDocument()
  ...
  it('does not display a badge when value is held', () => {
    render(<AttendanceStatusSelect value='held' onChange={() => {}} />)
    expect(
      screen.queryByText('Schülerabsenz (entschuldigt)'),
    ).not.toBeInTheDocument()
    expect(
