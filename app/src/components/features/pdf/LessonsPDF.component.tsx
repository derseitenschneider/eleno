  ...
  return (
    <Document>
      <Page size="A4" orientation="portrait" style={styles.page}>
        <Svg width="128" height="111" viewBox="0 0 128 111" style={styles.logo}>
          <Path
        ...
        <View style={styles.header}>
          <Text>Lektionsliste {studentFullName}</Text>
        </View>
        <Text
          style={styles.headerAfterOne}
          render={({ pageNumber }) =>
            pageNumber > 1 ? `Lektionsliste ${studentFullName}` : ''
          }
          fixed
        />

        <View style={[styles.table, styles.colHeader]}>
        ...
        {sanitizedLessons?.map((lesson, index) => (
          <View
            style={
              index % 2 === 1
                ? [styles.table, styles.row]
                : [styles.table, styles.row, styles.rowUneven]
            }
            wrap={false}
            key={lesson.id}
          >
