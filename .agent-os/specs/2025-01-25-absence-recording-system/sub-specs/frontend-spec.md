...
export function CreateLessonForm() {
  ...
  // NEW: Lesson status state
  const [lessonStatus, setLessonStatus] = useState<'held' | 'student_absent' | 'teacher_absent'>('held')
  ...
  const handleStatusChange = (newStatus: 'held' | 'student_absent' | 'teacher_absent') => {
      ...
      if (existingDraft) {
        return prev.map((draft) =>
          draft[typeField] === currentLessonHolder?.holder.id
            ? { ...draft, lesson_status: newStatus, date }
            : draft,
        )
      }
...
export function CreateLessonForm() {
  ...
  const lessonStatus = form.watch('lesson_status')
